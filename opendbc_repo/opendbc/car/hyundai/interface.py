...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret: structs.CarParams, candidate, fingerprint, car_fw, experimental_long, docs) -> structs.CarParams:
    ...
    if candidate == CAR.KIA_NIRO_EV_2ND_GEN:
      ...
      ret.lateralTuning.pid.kpV = [0.6, 0.3, 0.2]     # 0.2 from KisaPilot Tune PID_A, 0.6 from KisaPilot Tune_C (testing less aggressive lane change at higher speeds)
      ret.lateralTuning.pid.kiV = [0.1, 0.07, 0.05]   # 0.05 from KisaPilot Tune PID_A, 0.1 from KisaPilot Tune_C (testing for low speed tighter turns)
